...
``Listing 13. A b-thread that implements the requirement that X and O play alternatively.``

.. literalinclude:: Examples_code/TTT_listing2.js
  :linenos:
  :language: javascript

``Listing 14. A b-thread that implements the requirement that a square may only be marked once. Given the variables row and col that represent the coordinates of a square on the grid, the b-thread waits for a move on that square and blocks further moves on the same square.``

.. literalinclude:: Examples_code/TTT_listing3.js
  :linenos:
  :language: javascript

``Listing 15. A b-thread that implements the draw ending condition. It waits for nine moves and then requests to announce a draw. The variable move is a representation of all events of type Move. The number 90 specifies the priority of the request.``

.. literalinclude:: Examples_code/TTT_listing4.js
  :linenos:
  :language: javascript

``Listing 16. A b-thread that implements the X winning condition. Given a permutation p and a line l (row, column, or diagonal), the b-thread waits for three X events on the line, in the order specified by the permutation, and then requests to announce that X wins the game.``

.. literalinclude:: Examples_code/TTT_listing5.js
  :linenos:
  :language: javascript

``Listing 17. A b-thread that implements the O winning condition. Given a permutation p and a line l (row, column, or diagonal), the b-thread waits for three O events on the line, in the order specified by the permutation, and then requests to announce that O wins the game.``

.. literalinclude:: Examples_code/TTT_listing6.js
  :linenos:
  :language: javascript

``Listing 18. A b-thread that implements the detection of user’s (player X) click.``

.. literalinclude:: Examples_code/TTT_listing7.js
  :linenos:
  :language: javascript

``Listing 19. A b-thread that implements the requirement that no moves are allowed once the game ends.``
...
``Listing 20. A b-thread that implements the thumb-rule that, if no other thumbrule applies, it is best to put an O in the center square.``

.. literalinclude:: Examples_code/TTT_listing9.js
  :linenos:
  :language: javascript

``Listing 21. A b-thread that implements the thumb-rule that, if no other thumbrule applies, and the center square is taken, it is best to put an O in a corner square.``

.. literalinclude:: Examples_code/TTT_listing10.js
  :linenos:
  :language: javascript

``Listing 22. A b-thread that implements the thumb-rule that, if no other thumbrule applies, and the center and all corner squares are taken, put an O in a side square.``
